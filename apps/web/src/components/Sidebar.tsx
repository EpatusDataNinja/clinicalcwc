...
const bottomItems: NavItem[] = [
  { id: 'nav-manual', label: 'User Manual', href: '/user-manual', icon: HelpCircle },
...
export default function Sidebar({ isMobile, onMobileClose }: SidebarProps) {
  ...
  const activeCasesCount = cases.filter(
    (c) => c.status === 'active' || c.status === 'critical'
  ).length;
  const criticalCasesCount = cases.filter((c) => c.status === 'critical').length;
  const overdueTasksCount = tasks.filter(
    (t) => !t.completed && new Date(t.dueAt) < new Date()
  ).length;
  ...
  return (
    ...
    >
      ...
      {/* Sync Status */}
      <div
        className={`flex items-center gap-2 mx-3 mt-3 mb-1 px-2 py-1.5 rounded-md transition-all shrink-0 ${
          isOnline
