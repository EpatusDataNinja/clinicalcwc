...
import ClinicalLockScreen from '@/components/ClinicalLockScreen';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const isLocked = useCaseStore((state: CaseStore) => state.isLocked);
  const encryptionPasscode = useCaseStore((state: CaseStore) => state.encryptionPasscode);
  ...
  if (authToken && (isLocked || !encryptionPasscode)) {
    return (
          ...
          {dataStatus === 'error' ? (
            <div className="flex flex-col items-center justify-center h-full space-y-4 px-6 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-sm">
                {initError ||
                  'There was a problem loading your clinical workspace. Please try refreshing the page.'}
              </p>
              <button
